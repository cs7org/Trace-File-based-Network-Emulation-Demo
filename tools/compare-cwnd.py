...
def main():
    series_simulation = load_series(file_simulation, scale=1, offset=-3)
    ...
    ax.plot(series_simulation["receive_time"], series_simulation["sock_cwnd"], label="ns-3 Simulation", color=SIMULATION, linewidth=2)
    ax.plot(series_emulation["receive_time"], series_emulation["sock_cwnd"], label="Emulation", color=EMULATION, linewidth=2)
    ax.plot(series_testbed["receive_time"], series_testbed["sock_cwnd"], label="Real Testbed", color=TESTBED, linewidth=2)
