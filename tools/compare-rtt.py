...
def load_series(file, scale = 1, offset = 0):
    ...
    df["sock_rtt"] = (df["sock_rtt"] / 1e6) * scale # ns -> ms

    return df

def main():
    series_simulation = load_series(file_simulation, scale=1)
    series_emulation = load_series(file_emulation, scale=1e3, offset=5)
    series_testbed = load_series(file_testbed, scale=1e3)
